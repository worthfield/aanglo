...
const SingleStore = ({shop}) => {
  ...
  return (
    ...
    <div className="p-4">
      <h3 className="text-gray-900 text-sm font-medium sm:text-lg">{shop?.name}</h3>
      <p className="text-gray-600 text-sm mt-2">{shop?.description?.length>="10"?`${shop?.description.substring(0,50)}...`:shop?.description}</p>
    </div>
  </div>
  )
}
